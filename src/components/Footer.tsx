...
export function Footer() {
  const { whatsapp, email, instagram, tiktok, youtube } = useSettings();
  const waLink = `https://wa.me/${whatsapp || "6281234567890"}`;
      ...
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 py-9 grid gap-7 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          ...
          <ul className="space-y-2 text-[13px] text-[var(--jx-muted)]">
            <li>
              <a
                href={`mailto:${email || "halo@juevix.net"}`}
                className="hover:text-[var(--jx-neon-600)]"
              >
        ...
        <div>
          ...
          <div className="flex gap-2">
            <a href={instagram ? `https://instagram.com/${instagram.replace("@", "")}` : "#"} className="w-9 h-9 rounded-lg grid place-items-center text-white hover:opacity-80 transition-opacity" style={{ background: "#12241d" }}>
            ...
            <a href={tiktok ? `https://tiktok.com/${tiktok.replace("@", "")}` : "#"} className="w-9 h-9 rounded-lg grid place-items-center text-white hover:opacity-80 transition-opacity" style={{ background: "#12241d" }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                <path d="M16.6 5.82s.51.5 0 0A4.278 4.278 0 0 1 15.54 3h-3.09v12.4a2.592 2.592 0 0 1-2.59 2.5c-1.42 0-2.6-1.16-2.6-2.6 0-1.72 1.66-3.01 3.37-2.48V9.66c-3.45-.46-6.47 2.22-6.47 5.64 0 3.33 2.76 5.7 5.69 5.7 3.14 0 5.69-2.55 5.69-5.7V9.01a7.35 7.35 0 0 0 4.3 1.38V7.3s-1.88.09-3.24-1.48Z"/>
              </svg>
            </a>
            <a href={youtube ? `https://youtube.com/@${youtube.replace("@", "")}` : "#"} className="w-9 h-9 rounded-lg grid place-items-center text-white hover:opacity-80 transition-opacity" style={{ background: "#12241d" }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                <path d="M10 15l5.19-3L10 9v6m11.56-7.83c.13.47.22 1.1.28 1.9.07.8.1 1.49.1 2.09L22 12c0 2.19-.16 3.8-.44 4.83-.25.9-.83 1.48-1.73 1.73-.47.13-1.33.22-2.65.28-1.3.07-2.49.1-3.59.1L12 19c-4.19 0-6.8-.16-7.83-.44-.9-.25-1.48-.83-1.73-1.73-.13-.47-.22-1.1-.28-1.9-.07-.8-.1-1.49-.1-2.09L2 12c0-2.19.16-3.8.44-4.83.25-.9.83-1.48 1.73-1.73.47-.13 1.33-.22 2.65-.28 1.3-.07 2.49-.1 3.59-.1L12 5c4.19 0 6.8.16 7.83.44.9.25 1.48.83 1.73 1.73Z"/>
              </svg>
            </a>
            <a href={waLink} className="w-9 h-9 rounded-lg grid place-items-center text-white hover:opacity-80 transition-opacity" style={{ background: "#12241d" }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
