  ...
  return (
    <main className="mx-auto max-w-[1200px] px-3 sm:px-5 py-4 pb-28 lg:pb-8">
      ...
      <nav className="flex items-center gap-2 text-[12px] text-[var(--jx-muted)] font-semibold mb-3 px-1 flex-wrap">
        ...
        <span>›</span>
        <Link href="/" className="hover:text-[var(--jx-neon-600)]">{game.category}</Link>
        <span>›</span>
        <span className="text-[var(--jx-ink)]">{game.name}</span>
      </nav>
        ...
        <div className="grid md:grid-cols-[280px_minmax(0,1fr)] items-stretch">
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="flex items-center gap-3 mt-2 text-[13px] font-semibold">
              ...
              <span className="text-white/50">{game.reviews} review</span>
              {game.developer && <span className="text-white/50 hidden sm:inline">• {game.developer}</span>}
            </div>
            <p className="text-white/65 mt-3 text-[13px] sm:text-sm max-w-xl leading-relaxed">
              Top up {game.name} langsung masuk ke akun kamu dalam hitungan detik.
          ...
          <section className="jx-panel p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-2.5">
              {nominals.map((n) => (
                ...
                >
                  {n.badge && (
                    <span className={`absolute top-2 right-2 jx-badge ${n.badge_type === "ok" ? "jx-ok" : "jx-wait"}`}>
                      {n.badge}
                    </span>
                  )}
            ...
            {paymentCategories.map((cat) => (
              <div key={cat.label} className="mb-5">
                ...
                <div className={`grid gap-2.5 ${
                  cat.methods.length <= 2 ? "sm:grid-cols-2" : "grid-cols-2 sm:grid-cols-4"
                }`}>
                  {cat.methods.map((m) => (
                    <button
                      key={m.name}
          ...
          <div className="jx-panel p-5 lg:sticky jx-sticky" style={{ top: 84 }}>
            ...
            <div className="flex items-center gap-3 pb-4 border-b border-[var(--jx-line)]">
              ...
              <div className="min-w-0">
                <p className="font-bold text-[13px] truncate">{game.name}</p>
                <p className="text-[11px] text-[var(--jx-muted)]">{game.developer || game.category}</p>
              </div>
            </div>
      ...
      <section id="cara" className="mt-8">
        <h2 className="font-display text-lg sm:text-xl font-extrabold mb-1">Cara Top Up {game.name}</h2>
      ...
      <section className="mt-8 grid lg:grid-cols-[minmax(0,1fr)_340px] gap-4 items-start">
        <div>
          <h2 className="font-display text-lg sm:text-xl font-extrabold mb-1">FAQ Top Up {game.name}</h2>
