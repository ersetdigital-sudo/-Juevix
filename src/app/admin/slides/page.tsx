...
export default function AdminSlides() {
  ...
  const handleEdit = (s: Slide) => {
    setForm({ image: s.image, alt: s.alt, href: s.href, sort_order: s.sort_order, is_active: s.is_active });
    setEditId(s.id);
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-6">
      <ConfirmModal
        open={!!deleteTarget}
        title="Hapus Slide?"
        description={`Banner "${deleteTarget?.alt}" akan dihapus permanen dan tidak bisa dikembalikan.`}
